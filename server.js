...
app.post("/api/login", async (req, res) => {
  ...
  if (username === "tutor" && password === "tutor") {
    return res.json({ role: "tutor", redirectUrl: "/tutor.html" });
  }
...
// Aktualisierte Route für Tutor mit Such- und Filterfunktion
app.get("/api/tickets", async (req, res) => {
  try {
    ...
    console.error("Fehler beim Abrufen der Tickets für den Tutor:", err);
    res.status(500).json({ message: "Fehler beim Abrufen der Tickets" });
  }
});
